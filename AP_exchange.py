...
# EAPoL_header = str(sniff(filter = "ip and host 192.168.43.49", prn = packet_print)).encode('hex')
# print EAPoL_header
...
a = IP(src = "192.168.43.40", dst = "192.168.43.49") /Ether(dst = "50:b7:c3:e5:7d:09")/ Raw(load = message1)
send(a)
gc = inc(gc)
...
def flag_setter():
    ...
    a = IP(src = "192.168.43.40", dst = "192.168.43.49") /Ether(dst = "50:b7:c3:e5:7d:09")/ Raw(load = message3)
    send(a)
    # gc = inc(gc)
    # Message 4
...
def packet_print_1():
    MIC = hmac.new(KCK, packet.load, hashlib.sha512).digest()        
    if packet.load == MIC:
        flag_setter()
...
EAPoL_header = str(sniff(filter = "ip and host 192.168.43.49", prn = packet_print_1)).encode('hex')
# # print EAPoL_header
# # MIC = hmac.new(KCK, EAPoL_header, hashlib.sha512).digest()
